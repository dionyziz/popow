...
We now evaluate the cost of NIPoPoWs when used in realistic blockchain applications.
...
currencies, but this introduces reliance on a third party.
...
but funds are raised in another. The \emph{source blockchain} is
where fund-raising will take place, while the \emph{destination} is the
...
creates the ERC-20-style smart contract in the destination blockchain. Someone
wishing to participate in the ICO transfers funds into the
designated account on the source blockchain. Once their transfer
becomes confirmed, the payer generates a NIPoPoW about the transaction
