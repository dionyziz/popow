...
The obvious way to upgrade a cryptocurrency to support our protocol is a hard
fork: the block header is modified to include the interlink structure, and the
...
construction requires including the interlink data structure not in the block
header, but in the coinbase transaction. It is enough to only store a hash of
the interlink structure. The only requirement for the PoPoWs to work is that the
PoW commits to all the pointers within interlink so that the adversary cannot
cause a chain reorg. If we take that route, then each PoPoW will be required to
...
observe that including such proofs-of-inclusion will only increase the PoPoW
...
the blockchain include the interlink structure. Given that intuitively these
...
interlink pointer blindly it now calls the helper function called
\textit{followUp}, shown in Algorithm~\ref{alg.nipopow-velvet-follow}. It
accepts a block $B$ and a level $\mu$ and creates a connection from $B$ back to
the most recent preceding level-$\mu$ block, by following the
interlink pointer if it is correct. Otherwise, it follows the
...
simply applies the velvet
$\chain\upchain$ % '
operator and includes the auxiliary connecting nodes
...
\textbf{Supporting clients with different beliefs.}
