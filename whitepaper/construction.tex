...
level $\mu = 2$. For the $m$-sized suffix of that level, $5$ blocks of
superblock level $1$ are included to span the same range. For the last $3$
blocks of the $1$-superchain, blocks of level $0$ spanning the same range are
included for support. Note that the superchain at a lower levels may reach
closer to the end of the blockchain than a higher level.

\begin{figure}[h]
    \caption{
    NIPoPoW \emph{prefix} $\pi$ for $m = 3$.
    }
    \centering
    \iftwocolumn
